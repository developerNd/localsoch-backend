...
async function resetBusinessCategories() {
  try {
    ...
    const vendorsResponse = await axios.get(`${API_URL}/api/vendors?populate=businessCategory`);
    ...
    for (const oldCategory of currentCategories) {
      try {
        await axios.delete(`${API_URL}/api/business-categories/${oldCategory.id}`);
    ...
    const verifyResponse = await axios.get(`${API_URL}/api/business-categories`);
    ...
    const verifyVendorsResponse = await axios.get(`${API_URL}/api/vendors?populate=businessCategory`);
