...
async function createProductsFinal() {
  try {
    console.log('📦 Creating products with proper permissions...');
    ...
    // Step 2: Update permissions for all roles
    console.log('\n🔐 Step 2: Updating permissions for all roles...');
    ...
    try {
      ...
      console.log('❌ Error updating public role:', error.response?.status);
    }
    ...
    try {
      ...
      console.log('❌ Error updating authenticated role:', error.response?.status);
    }
    
    // Step 3: Get categories and vendors
    ...
    console.log('\n🧪 Step 6: Testing the final setup...');
...
// Run the script
createProductsFinal().then(success => {
  if (success) {
    ...
    console.log('6. Browse products by category and vendor');
  } else {
